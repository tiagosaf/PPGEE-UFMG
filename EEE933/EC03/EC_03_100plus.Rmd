...
Como ($p_a^a<<\alpha$), é possível rejeitar $H_{a0}^a$ em detrimento da hipótese alternativa.
...
### Conclusões
É possível concluir com um nível de confiança de 95\% que o algoritmo proposto implica em uma degradação considerável de acurácia. De fato, o intervalo de confiança para a diferença das médias da acurácia se encontra na região de rejeição afastado da região crítica. O tamanho de efeito prático foi $\delta_a^{\ast}=0.03$ e o número de instâncias considerado no teste foi de $n_t=102$. Uma análise conservadora foi utilizada na variância amostral considerada, sendo que utilizou-se a maior variância amostral do intervalo de confiança para $\alpha=0.05$.
